...
class ComputerVisionPlayground(tk.Grid):
    ...
    def __init__(self):
        ...
        """
        This comment applies ONLY to the line of code as follows: the cv2.CAP_ANY used to
        say cv2.CAP_DSHOW... that flag reperesents "preferred Capture API backends to use."
        The generic code tries to enforce a specific reader implementation that this computer must not accept...
        To correct, let openCV choose for itself with the flag "cv2.CAP_ANY"
        """

        self.vs = cv2.VideoCapture(at.VIDEO_SOURCE, cv2.CAP_ANY)
        # frame dimensions
        ...
        try:
            while not self.stopEvent.is_set():
                ...
                # update image with new frame
                    ...
                    print("updating normal image with new frame")
                    # update image
                    self.normal_frame.configure(image=frame_clr)
                    # save image
                    self.normal_frame.image = frame_clr

                    # Derek Try Updating Image with New Frame
                    self.normal_frame.grid(row=at.HEADER_N_FEED_ROW + 1,
                                           column=at.HEADER_N_FEED_COLUMN,
                                           rowspan=at.IMAGE_ROWSPAN,
                                           columnspan=at.IMAGE_COLUMNSPAN,
